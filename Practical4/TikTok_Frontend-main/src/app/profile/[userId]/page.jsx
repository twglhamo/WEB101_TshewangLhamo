...
export default function ProfilePage() {
  ...
  // ✅ Fixed: all references use `userId` not undefined `id`
  const refreshProfileData = async () => {
    try {
      ...
      if (isAuthenticated && currentUser) {
        ...
        const followersList = Array.isArray(followersData) ? followersData : (followersData.followers || []);
        setFollowers(followersList);
        setIsFollowing(followersList.some(f => f.id === currentUser.id));
      }

      const followingData = await getUserFollowing(userId);
      setFollowing(Array.isArray(followingData) ? followingData : (followingData.following || []));
